...
export default function AuthModal({ supabase, onClose }) {
  ...
  const handleMagicLink = async () => {
    if (!email.trim()) { setMessage('Introduce un email primeiro'); return }
    setLoading(true)
    setMessage('')
    const { error } = await supabase.auth.signInWithOtp({ email })
    if (error) { setMessage(error.message); setLoading(false); return }
    setMessage('Link máxico enviado! Revisa o teu email.')
    setLoading(false)
  }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
        ...
        <div className="text-center mb-6">
          <div className="w-14 h-14 mx-auto mb-3 rounded-full bg-blue-600/30 border-2 border-blue-400/30 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} required placeholder="Contrasinal"
            className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500" />
          {message && <p className="text-xs text-blue-300 text-center">{message}</p>}
        ...
        <div className="relative my-4">
          ...
          <button onClick={handleGoogle}
            className="w-full bg-white hover:bg-slate-100 text-slate-900 font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 text-sm">
          ...
          <button onClick={handleMagicLink} disabled={loading}
            className="w-full bg-slate-700 hover:bg-slate-600 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 text-sm">
            <Send size={14} /> Link máxico
          </button>
        <p className="text-xs text-slate-500 text-center mt-4">
          {mode === 'login' ? 'Non tes conta? ' : 'Xa tes conta? '}
